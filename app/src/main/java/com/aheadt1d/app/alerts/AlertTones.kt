 ...
 * sound attribute - same reasoning as CriticalLowSiren's direct playback:
 * decouples "which sound plays" from "which channel this groups under", and
 * for urgent/signal-lost specifically, USAGE_ALARM audio attributes bypass
 * DND the same reliable way CriticalLowSiren's sound does (independent of
 * the fragile notification-policy-access permission - see CriticalLowSiren's
 * class doc for the incident that taught us this). Calm/warn tones
 * deliberately do NOT get that treatment - yellow-tier alerts are meant to
 * respect DND, not pierce it (see AlertChannels' own doc on that).
 *
...
object AlertTones {
    ...
    // Alarm-usage, same as CriticalLowSiren's sound - reliably bypasses DND
    // in every mode except "Total Silence", with no special permission
    // needed. Only for the urgent tier (red/signal-lost); calm/warn must
    // keep respecting DND, that's deliberate.
