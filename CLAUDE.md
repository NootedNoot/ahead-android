...
- *Red* — **no tone at all, in either branch** (2026-08-01, at the owner's request: across several lows in one day the repeated alarm-stream tone became punishing). Red is one ordinary DND-bypassing notification tier with copy/vibration gated on `LowAlertPhase` (`AlertThresholds.kt`; `AlertNotifier.showRedAlert`, `SpokenAlertText.red`) — vibration + voice only, no siren, no screen takeover. The RED voice category **bypasses the voice master toggle** (`VoiceAlertEngine.UNGATED_CATEGORIES`) — otherwise removing the tone would silently reduce red to vibration only whenever that toggle happened to be off, which it was. Its channel is `setSound(null, null)` but still `setBypassDnd(true)` with a vibration pattern: no sound is not no interruption.
...
#### Low-side alert state: value gates the label, rate only gates urgency (2026-09-23)

Real reported bug: a 79 mg/dL reading at +0.8 mg/dL/min still showed **"Still low: 79 mg/dL, rising"** — 79 is already above the app's own 70 mg/dL low threshold, so calling it "low" at all was factually wrong. Root cause: the old `LOW_RED_CLEAR_HYSTERESIS` was a flat 80 mg/dL band, not the app's real 70 mg/dL threshold — a value could sit fully above the threshold Ahead itself uses everywhere else and still get held under the old "still low" copy for another 10 points.

Fixed by decoupling two questions that used to be conflated into one boolean (`recovering: Boolean`):

- **State** (is this episode currently low): now gates strictly on `LOW_HIGH_SPLIT` (70, or the 15-min projection — `AlertThresholds.isStillLow`-equivalent logic lives in `AlertCoordinator.isStillLow`), no rate dependency. Value/projection ≥ 70 is never labeled "low," full stop.
- **Urgency** (how loud the copy/vibration is): `alerts/LowAlertPhase` (`AlertThresholds.kt`) — `URGENT` (still low, **2+ consecutive** readings at/below `SeverityEngine.RATE_FALLING_TRIGGER`, not one blip), `STANDARD` (still low, flat/mild negative — a real ongoing low, but not worth re-escalating tone over noise), `RISING` (still low but rate positive and holding — informational, avoids prompting an over-correction), `RECOVERING` (value ≥ 70 but not yet confirmed stable — see below). `AlertCoordinator.updateLowPhase`/`currentLowPhase` are the one shared decision point both the actively-red path (`fireRedIfWarranted`) and the held-past-red path (`handleReading`'s clear-buffer block) use, so they can never disagree.

A **new "recovering, not yet stable" buffer** replaces the old flat mg/dL band: once value/projection crosses back to ≥ 70, the episode doesn't fully clear (cancel the alert, let severity flow through) until `LOW_STABILITY_READINGS_REQUIRED` (2) **consecutive** readings hold or keep climbing. This is what catches a bounce mid-drop instead of prematurely signaling recovery on the first good tick — real case that motivated it: 79 → 87 → 89 → 83 @ -1.2 mg/dL/min, where the first two readings alone would have looked like recovery. A reading that breaks an already-building streak (drops back under 70, or the rate goes negative again) re-alerts immediately, same urgency as a fresh low — including the case where severity had already re-scored all the way back to `"red"`: `handleRedTransition`'s `forceFire` also triggers whenever `KEY_LOW_STABILITY_STREAK > 0` at the time of re-entry, not just on `enteringRed`, since `KEY_LAST_SEVERITY` stays `"red"` in prefs for the entire duration an episode is held (see the clear-buffer block returning early without advancing it) — `enteringRed` alone can never detect "this was recovering and just reversed."

Being reactive rather than predictive, this streak buffer is a meaningful reduction in false-clears, not a guarantee against every possible bounce shape — a bounce that reverses on its 3rd+ reading, after the streak has already cleared, is caught by the ordinary fresh-episode/re-scoring path instead, not by this buffer specifically.

Streaks (`KEY_LOW_WORSENING_STREAK`, `KEY_LOW_STABILITY_STREAK`) only advance on a genuinely NEW reading (`handleRedTransition`), never on a same-date 60s-tick re-emission (`handleRedHeartbeat`, `mutateLowStreak = false`) — otherwise one real CGM sample re-evaluated every tick could satisfy "2 consecutive readings" within a couple of minutes, defeating the point. A **brand-new episode** (`forceFire`) still always falls back to the old simple rate-sign split (`RISING` if rising, else `URGENT`) rather than the streak-gated `STANDARD`/`URGENT` split — there's no history yet to judge "sustained" against, and a first-time critical low should never be undersold as merely "Low: X mg/dL" for lack of streak.

Rebound highs are untouched by any of this — they're a fully separate code path (the high-side branches of `fireRedIfWarranted`/the clear-hysteresis block, still the old flat `HIGH_RED_CLEAR_HYSTERESIS_*` band, per the owner's explicit "keep fully separate" ask).
